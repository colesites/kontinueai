...
export function SidebarHeaderSection({
	...
	return (
		<SidebarHeader className="border-b border-foreground/6 -mx-2 px-5 py-3.5 gap-3.5">
			...
			<div className="glow-rail-focus relative rounded-xl">
				<SearchIcon
					size={14}
					className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 z-10 text-muted-foreground/70"
				/>
				<SidebarInput
					...
					className={cn(
						...
						"focus-visible:bg-foreground/6 focus-visible:ring-0 focus-visible:border-transparent",
					)}
				/>
				{searchQuery ? (
					<button
			...
			<div className="flex flex-col gap-2">
				...
				{/* Row 3: expandable product menu */}
				<DropdownMenu>
					<DropdownMenuTrigger asChild>
						<button
							type="button"
							aria-label="More Kontinue products"
							className={cn(
								"group flex flex-1 items-center justify-center gap-2 rounded-xl px-4 py-2 text-[13px] font-medium transition-all duration-200",
								"surface-inset text-muted-foreground",
								"hover:bg-foreground/8 hover:text-foreground hover:scale-[1.015]",
						...
						>
							<MoreHorizontal
								size={15}
								className="transition-transform duration-300 group-hover:tracking-widest"
							/>
							<span>More</span>
						</button>
					</DropdownMenuTrigger>
