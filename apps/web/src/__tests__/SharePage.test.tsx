...
describe("SharePage - Unit Tests", () => {
	...
	test("displays 404 error for invalid chatId", async () => {
		...
		const errorMessage = Array.from(container.querySelectorAll("p")).find((p) =>
			p.textContent?.includes("doesn't exist or has been deleted"),
		);
	...
	test("displays empty state when conversation has no messages", async () => {
		...
		const emptyMessage = Array.from(container.querySelectorAll("p")).find((p) =>
			p.textContent?.includes("no messages yet"),
		);
	...
	test("renders multiple messages in correct order", async () => {
		...
		await waitFor(
			() => {
				const allText = container.textContent || "";
				if (!allText.includes("First message"))
					throw new Error("Messages not rendered");
			},
			{ timeout: 1000 },
		);
