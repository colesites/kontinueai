...
describe("ShareModal - Property-Based Tests", () => {
	...
	test("Property 2: Modal Title Format - Feature: chat-share-feature, Property 2: Modal Title Format", () => {
		...
		fc.assert(
			fc.property(fc.string({ minLength: 1, maxLength: 100 }), (chatTitle) => {
				// Clean up before each render
				cleanup();
				...
				const expectedTitle = trimmedTitle
					? `Share ${trimmedTitle}`
					: "Share Conversation";
				...
				fc.asyncProperty(
					...
					async (chatId) => {
						...
						await waitFor(
							() => {
								if (clipboardContent === "")
									throw new Error("Clipboard not updated");
							},
							{ timeout: 1000 },
						);
			...
			await fc.assert(
				fc.asyncProperty(
					...
					async (chatId) => {
						...
						const buttonAfterClick = document.querySelector(
							'[data-testid="copy-link-button"]',
						) as HTMLElement;
