...
function setNavigatorShare(value: Navigator["share"] | undefined) {
	if (value) {
...
describe("ShareModal - Unit Tests", () => {
	...
	test("social sharing options are present when Web Share API is available", () => {
		...
		const originalShare = navigator.share;
		setNavigatorShare(async () => undefined);
	...
	test("share API called with correct parameters", async () => {
		...
		try {
			...
			const capturedData: { title?: string; url?: string } = {};
			Object.defineProperty(navigator, "share", {
			...
			expect(capturedData.url).toBe(
				"http://localhost:3000/share/test-chat-123",
			);
