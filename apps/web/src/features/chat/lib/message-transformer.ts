...
function extractToolGeneratedImageBase64(
...
export function mapStoredGeneratedImages(
	chatFiles:
		| Array<{ fileType: string; messageId?: string; blobUrl: string }>
		| undefined,
...
export function mapImportedFlags(
	messages:
		| Array<{ _id: string; metadata?: { isImported?: boolean } }>
		| undefined,
...
export function mergeImportedChunks<
	T extends {
		role: "user" | "assistant";
		content: string;
		imageParts: string[];
