		...
		try {
			...
			recognition.onresult = (event) => {
				...
				const combinedTranscript =
					`${refs.speechFinalTextRef.current} ${interim}`.trim();
				setInputValue(
					mergeSpeechText(refs.speechBaseTextRef.current, combinedTranscript),
				);
			};
				...
				if (code === "no-speech") {
					refs.forceAdvanceLanguageRef.current =
						...
						refs.languageIndexRef.current <
							refs.languageCandidatesRef.current.length - 1;
					return;
				}
