...
const HOUR_MARKS = Array.from({ length: 12 }, (_, hour) => hour);
...
function AnalogClock({
	...
	return (
		...
		>
			...
			{/* Hour ticks */}
			{HOUR_MARKS.map((hourMark) => {
				const angle = hourMark * 30 * (Math.PI / 180);
				const isQuarter = hourMark % 3 === 0;
			...
			<line
				...
				style={{
					...
					filter:
						"drop-shadow(0 0 3px color-mix(in oklch, var(--primary) 50%, transparent))",
				}}
			/>
