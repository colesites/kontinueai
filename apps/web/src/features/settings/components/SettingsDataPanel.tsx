...
> = {
	json: {
		label: "JSON",
		description:
			"Single machine-readable file with conversations, memories, summaries.",
		icon: FileJson,
	},
	markdown: {
		label: "Markdown",
	...
	zip: {
		label: "ZIP archive",
		description:
			"JSON + one Markdown file per conversation + README. Recommended.",
		icon: FileArchive,
	},
};
		...
		<div className="space-y-8">
			<div>
				...
				<p className="mt-2 text-sm text-muted-foreground">
					Export everything Kontinue AI has stored about you, or bring your
					history over from another assistant.
				</p>
			</div>
...
type ExportRow = NonNullable<
	ReturnType<typeof useQuery<typeof api.exports.listExports>>
>[number];
	...
	return (
		<section className="space-y-4">
			<div className="space-y-1">
				...
				<p className="text-xs text-muted-foreground">
					We import the oldest and newest conversations first so you can chat
					with full context within seconds. The rest streams in quietly in the
					background.
				</p>
			</div>
		...
		<div className="rounded-xl surface-inset p-4 space-y-3">
			<div className="flex items-start justify-between gap-3">
				<div className="space-y-1">
					...
					<p className="text-xs text-muted-foreground">
						...
						{job.completedAt
							? ` · finished ${formatTimestamp(job.completedAt)}`
							: ""}
					</p>
				</div>
				{isActive ? (
			...
			<div className="space-y-1.5">
				...
				<div className="flex justify-between text-[11px] text-muted-foreground">
					<span>
						{job.processedConversations} / {job.totalConversations}{" "}
						conversations
					</span>
					<span>{percent}%</span>
				</div>
			...
			{phase1Ready && isActive ? (
				<div className="rounded-lg border border-primary/20 bg-primary/[0.04] px-3 py-2 text-xs">
					...
					Your most important memories are ready while the rest continues
					importing.
				</div>
			) : null}
