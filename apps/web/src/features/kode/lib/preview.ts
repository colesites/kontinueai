...
export function buildKodePreviewDocument(files: KodePreviewFile[]): string {
	...
	// The generated files use familiar local references. The preview is srcDoc,
	// so replace those references with inline content rather than issuing broken
	// requests to the Kontinue origin.
	html = html
