...
import { detectProvider, SharedLinkSchema } from "@repo/utils/url-safety";
...
export async function POST(request: NextRequest) {
	...
	headers.Origin =
		process.env.SCRAPER_REQUEST_ORIGIN ?? "http://localhost:3000";
	...
	try {
		...
		const data = (await scraperResponse
			.json()
			.catch(() => ({}))) as ScraperResponse;
