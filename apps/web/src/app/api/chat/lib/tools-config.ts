	...
	return tool({
		...
		inputSchema: z.object({
			...
			priority: z
				...
				.optional()
				.describe(
					"Task priority. Default 'medium'. Use 'urgent' only when explicitly time-critical.",
				),
	...
	return tool({
		...
		inputSchema: z.object({
			...
			repo: z
				...
				.optional()
				.describe(
					"Repository as 'owner/name' (e.g. 'vercel/next.js'). Required for everything except list_repos and search_code.",
				),
			title: z
				.string()
				.optional()
				.describe(
					"For create_issue/update_issue/create_pull_request: the title.",
				),
			body: z
				.string()
				.optional()
			...
			state: z
				...
				.optional()
				.describe(
					"For list_issues/list_pull_requests: filter by state (default open).",
				),
			labels: z
				.array(z.string())
				.optional()
			...
			path: z
				...
				.optional()
				.describe(
					"For get_file/create_or_update_file: the file path within the repo.",
				),
			content: z
				.string()
				.optional()
				.describe(
					"For create_or_update_file: the new file content (plain text).",
				),
			message: z
				.string()
				.optional()
			...
			branch: z
				...
				.optional()
				.describe(
					"For get_file/list_commits/create_or_update_file: the branch/ref. For create_branch: the NEW branch name.",
				),
			baseBranch: z
				.string()
				.optional()
				.describe(
					"For create_branch: the branch to fork from (defaults to the repo's default branch).",
				),
			head: z
				.string()
				.optional()
				.describe(
					"For create_pull_request: the branch containing your changes.",
				),
			base: z
				.string()
				.optional()
				.describe(
					"For create_pull_request: the branch to merge into (e.g. 'main').",
				),
			query: z
				.string()
				.optional()
			...
			limit: z
				...
				.optional()
				.describe(
					"Max items to RETURN for list actions (default 30). Does not affect list_repos' total count.",
				),
		}),
		execute: async ({
			action,
			...
			try {
				...
				if (action === "search_code") {
					if (!query)
						return {
							connected: true,
							error: "query is required for search_code.",
						};
					const res = await fetch(
				...
				if (action === "get_file") {
					if (!path)
						return { connected: true, error: "path is required for get_file." };
				...
				if (action === "create_issue") {
					if (!title)
						return {
							connected: true,
							error: "A title is required to create an issue.",
						};
				...
				if (action === "update_issue") {
					if (!issueNumber)
						return {
							connected: true,
							error: "issueNumber is required to update an issue.",
						};
					...
					if (Object.keys(patch).length === 0) {
						return {
							connected: true,
							error: "Provide title, body or labels to update.",
						};
					}
					const res = await fetch(
				...
				if (action === "close_issue" || action === "reopen_issue") {
					if (!issueNumber)
						return { connected: true, error: "issueNumber is required." };
					const res = await fetch(
						`${API}/repos/${repo}/issues/${issueNumber}`,
						{
							method: "PATCH",
				...
				if (action === "comment_issue") {
					if (!issueNumber || !body) {
						return {
							connected: true,
							error: "issueNumber and body are required to comment.",
						};
					}
					const res = await fetch(
				...
				if (action === "create_branch") {
					if (!branch)
						return {
							connected: true,
							error: "branch (the new name) is required.",
						};
					...
					if (!from) {
						...
						from = ((await repoRes.json()) as { default_branch: string })
							.default_branch;
					}
					...
					const sha = ((await refRes.json()) as { object: { sha: string } })
						.object.sha;
				...
				if (!title || !head || !base) {
					return {
						connected: true,
						error: "title, head and base are required to open a PR.",
					};
				}
	...
	return tool({
		...
		inputSchema: z.object({
			...
			query: z
				...
				.optional()
				.describe(
					"For search: text to match page/database titles. Empty returns recent items.",
				),
			parentPageId: z
				.string()
				.optional()
				.describe(
					"For create_page: the id of the parent page to nest the new page under.",
				),
			pageId: z
				.string()
				.optional()
				.describe(
					"For append/update_page: the id of the page (or database row) to act on.",
				),
			databaseId: z
				.string()
				.optional()
				.describe(
					"For get_database/query_database/create_database_item: the database id.",
				),
			title: z
				.string()
				.optional()
			...
			notionFilter: z
				...
				.optional()
				.describe(
					"For query_database: a raw Notion filter object (optional). See Notion API filter docs.",
				),
			notionSorts: z
				.any()
				.optional()
			...
			archived: z
				...
				.optional()
				.describe(
					"For update_page: set true to archive (delete) the page/row.",
				),
		}),
		execute: async ({
			action,
			...
			try {
				...
				if (!tokenResult)
					return { connected: false, error: "Notion is not connected." };
				const headers = {
					Authorization: `Bearer ${tokenResult.accessToken}`,
				...
				if (action === "search") {
					...
					if (!res.ok)
						return { connected: true, error: `Notion API error ${res.status}` };
					const data = (await res.json()) as {
						results: Array<{
							id: string;
							object?: string;
				...
				if (action === "read") {
					if (!pageId)
						return {
							connected: true,
							error: "pageId is required to read a page.",
						};
				...
				if (action === "create_page") {
					if (!parentPageId || !title) {
						return {
							connected: true,
							error: "parentPageId and title are required to create a page.",
						};
					}
				...
				if (action === "append") {
					if (!pageId || !content) {
						return {
							connected: true,
							error: "pageId and content are required to append.",
						};
					}
					...
					if (!appendResult.ok)
						return { connected: true, error: appendResult.error };
				...
				if (action === "get_database") {
					if (!databaseId) {
						return {
							connected: true,
							error: "databaseId is required for get_database.",
						};
					}
					const res = await fetch(
				...
				if (action === "create_database_item") {
					if (!databaseId || !properties) {
						return {
							connected: true,
							error:
								"databaseId and properties are required to create a database item.",
						};
					}
				...
				if (!pageId) {
					return {
						connected: true,
						error: "pageId is required to update a page.",
					};
				}
				const patch: Record<string, unknown> = {};
				...
				if (properties && Object.keys(properties).length > 0) {
					...
					if (!pageRes.ok)
						return {
							connected: true,
							error: `Notion API error ${pageRes.status}: ${await pageRes.text()}`,
						};
					const page = (await pageRes.json()) as {
					...
					if (!dbId) {
						return {
							connected: true,
							error:
								"This page is not a database row, so it has no editable properties. Use append to add content instead.",
						};
					}
					const built = await buildNotionPropertiesFromDb(
						dbId,
				...
				if (Object.keys(patch).length === 0) {
					return {
						connected: true,
						error: "Nothing to update — provide properties or archived.",
					};
				}
...
function summarizeNotionSchema(
	...
	for (const [name, def] of Object.entries(props)) {
		const options =
			def.select?.options ?? def.status?.options ?? def.multi_select?.options;
		out[name] = {
			type: def.type,
...
function simplifyNotionProperties(
	...
	for (const [name, raw] of Object.entries(props)) {
		...
		switch (type) {
			...
			case "number":
				out[name] = property.number;
				break;
			case "checkbox":
				out[name] = property.checkbox;
				break;
			case "select": {
			...
			case "email":
			case "phone_number":
				out[name] = property[type];
				break;
			case "people": {
				const people = property.people;
				out[name] = Array.isArray(people)
	...
	return tool({
		...
		execute: async ({ action, limit, deploymentId, name, target }) => {
			try {
				...
				if (!tokenResult)
					return { connected: false, error: "Vercel is not connected." };
				const authHeader = {
					Authorization: `Bearer ${tokenResult.accessToken}`,
				};
				...
				if (action === "redeploy") {
					if (!deploymentId || !name) {
						return {
							connected: true,
							error: "deploymentId and name are required to redeploy.",
						};
					}
					...
					if (!res.ok)
						return {
							connected: true,
							error: `Vercel API error ${res.status}: ${await res.text()}`,
						};
				...
				if (!res.ok)
					return { connected: true, error: `Vercel API error ${res.status}` };
				const data = (await res.json()) as {
					deployments: Array<{
						uid?: string;
						name: string;
			...
			try {
				...
				if (action === "create_task") {
					if (!content) {
						return {
							connected: true,
							error: "content is required to create a task.",
						};
					}
				...
				if (action === "update_task") {
					if (!taskId) {
						return {
							connected: true,
							error: "taskId is required to update a task.",
						};
					}
					...
					if (Object.keys(body).length === 0) {
						return {
							connected: true,
							error: "Nothing to update — provide a field to change.",
						};
					}
				...
				if (action === "delete_task") {
					if (!taskId) {
						return {
							connected: true,
							error: "taskId is required to delete a task.",
						};
					}
					const res = await fetch(`${BASE}/tasks/${taskId}`, {
						method: "DELETE",
				...
				if (!taskId) {
					return {
						connected: true,
						error: "taskId is required to complete a task.",
					};
				}
				...
				return {
					connected: false,
					error:
						error instanceof Error ? error.message : "Todoist request failed",
				};
			}
		},
	});
...
export async function getGoogleAccessToken(
	...
	try {
		await tokens.persistRefreshedToken(
			provider,
			json.access_token,
			tokenExpiresAt,
		);
		...
		console.error(
			`[oauth ${provider}] failed to persist refreshed token`,
			error,
		);
	}
	...
	return tool({
		...
		inputSchema: z.object({
			to: z
				...
				.optional()
				.describe(
					"Recipient email address(es), comma-separated. Leave empty if unknown.",
				),
			cc: z
				.string()
				.optional()
			...
			body: z
				.string()
				.describe("The full email body as plain text (no markdown)."),
		}),
	...
	return tool({
		...
		inputSchema: z.object({
			...
			query: z
				...
				.optional()
				.describe(
					"For search: a Gmail search query. See Gmail search operators.",
				),
			maxResults: z
				.number()
				.int()
				...
				.optional()
				.describe(
					"For search: how many message summaries to return (default 10). Does not affect estimatedTotal.",
				),
			messageId: z
				.string()
				.optional()
				.describe(
					"For read: the id of the message to fetch (from a prior search).",
				),
		}),
			...
			try {
				...
				if (action === "search") {
					...
					if (!res.ok)
						return {
							connected: true,
							error: await googleApiError(res, "Gmail"),
						};
					const data = (await res.json()) as {
					...
					const messages = await Promise.all(
						ids.map(async ({ id }) => {
							...
							const h = (name: string) =>
								md.payload?.headers?.find((x) => x.name.toLowerCase() === name)
									?.value;
							return {
								id,
								from: h("from"),
	...
	return tool({
		...
		inputSchema: z.object({
			...
			start: z
				...
				.optional()
				.describe(
					"For create: ISO 8601 start datetime (e.g. 2026-06-01T15:00:00Z).",
				),
			end: z.string().optional().describe("For create: ISO 8601 end datetime."),
			location: z
				.string()
				.optional()
				.describe("For create: optional location."),
		}),
		execute: async ({
			action,
			timeMin,
			...
			try {
				...
				if (action === "list") {
					...
					if (!res.ok)
						return {
							connected: true,
							error: await googleApiError(res, "Calendar"),
						};
					const data = (await res.json()) as {
						items?: Array<{
							id: string;
	...
	return tool({
		...
		inputSchema: z.object({
			...
			fileId: z
				...
				.optional()
				.describe(
					"For read: the id of the file to read (from a prior search).",
				),
		}),
		execute: async ({ action, query, fileId }) => {
			try {
				...
				if (!meta.ok)
					return {
						connected: true,
						error: await googleApiError(meta, "Drive"),
					};
				...
				return {
					connected: false,
					error:
						error instanceof Error ? error.message : "Drive request failed",
				};
			}
		},
	});
