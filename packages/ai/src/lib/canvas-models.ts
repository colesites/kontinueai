...
const K_IMAGE_MODEL: CanvasModel = {
	...
	// K-Image is available inside the Pro/Max Canvas workspace and is metered.
	isFree: false,
};
...
// Durations in seconds. K-Video is available only on Pro and Max.
export type CanvasPlanTier = "free" | "starter" | "plus" | "pro" | "max";

export const K_VIDEO_PLAN_LIMITS: Record<
	CanvasPlanTier,
	{ durations: number[]; maxResolution: string }
> = {
	free: { durations: [], maxResolution: "1280x720" },
	starter: { durations: [], maxResolution: "1280x720" },
	plus: { durations: [], maxResolution: "1280x720" },
	pro: { durations: [5, 10, 15, 30, 60], maxResolution: "1920x1080" },
	max: {
...
export function clampKVideoDuration(
	tier: CanvasPlanTier,
	duration: number,
): number {
...
export function clampKVideoResolution(
	tier: CanvasPlanTier,
	resolution: string,
): string {
	const maxRank = RESOLUTION_RANK[K_VIDEO_PLAN_LIMITS[tier].maxResolution] ?? 1;
