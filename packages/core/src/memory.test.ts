...
describe("memory helpers", () => {
	test("resolves memory limits by plan", () => {
		...
		expect(getMemoryLimitBytesForPlan("starter_plan")).toBe(5 * 1024 * 1024);
		expect(getMemoryLimitBytesForPlan("plus_plan")).toBe(100 * 1024 * 1024);
		expect(getMemoryLimitBytesForPlan("pro_plan")).toBe(500 * 1024 * 1024);
		expect(getMemoryLimitBytesForPlan("max_plan")).toBe(2 * 1024 * 1024 * 1024);
	});
	...
	test("tokenizes keywords for retrieval", () => {
		expect(
			tokenizeForKeywordSearch(
				"My startup is Kontinue AI and I study statistics",
			),
		).toEqual(
