...
import { planTierFromPersisted } from "./plan-tier";
import { PLAN_DEFINITIONS } from "./plan-config";

export const MEMORY_PLAN_LIMIT_BYTES = {
	free: PLAN_DEFINITIONS.free.memoryBytes,
	starter: PLAN_DEFINITIONS.starter.memoryBytes,
	plus: PLAN_DEFINITIONS.plus.memoryBytes,
	pro: PLAN_DEFINITIONS.pro.memoryBytes,
	max: PLAN_DEFINITIONS.max.memoryBytes,
...
export function getMemoryLimitBytesForPlan(plan?: string | null): number {
	const tier = planTierFromPersisted(plan);
