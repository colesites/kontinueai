...
export async function extractClaude(page: Page): Promise<ExtractResult> {
  await page
    .waitForSelector(
  ...
  const result = await page.evaluate(() => {
    ...
    function htmlToMarkdown(el: Element): string {
      ...
      for (const block of Array.from(clone.querySelectorAll("p, div, li, h1, h2, h3, h4, h5, h6, blockquote"))) {
        ...
        if (b.textContent && b.textContent.trim()) {
          b.replaceWith(document.createTextNode(prefix + b.textContent + "\n"));
        }
      }
      return (clone.textContent ?? "")
        .replace(/Log in to see interactive visuals/gi, "")
        .replace(/\n{3,}/g, "\n\n")
        .trim();
    }
    ...
    // Each conversation turn is wrapped in [data-test-render-count]. The turn's
    // accessibility text begins with "You said:" or "Claude responded:".
    ...
    for (const turn of turns) {
      ...
      const assistantEl = turn.querySelector(".font-claude-response");
      ...
      if (userEl) {
        ...
        const lead = (turn.textContent ?? "").trimStart().slice(0, 24).toLowerCase();
        if (lead.startsWith("you said")) role = "user";
        else if (lead.startsWith("claude responded") || lead.startsWith("claude said")) role = "assistant";
        contentEl = turn;
      }

      if (!role || !contentEl) continue;
      ...
      messages.push({
        role,
        content,
        codes: collectCode(contentEl),
        images: collectImages(contentEl),
        files: [],
      });
    }

    // Fallback: turn wrappers absent — use direct message selectors in DOM order.
