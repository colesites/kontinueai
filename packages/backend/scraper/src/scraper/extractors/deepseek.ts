...
export async function extractDeepSeek(page: Page): Promise<ExtractResult> {
  await page.waitForSelector(".ds-message", { timeout: 20000 }).catch(() => null);
  ...
  const result = await page.evaluate(() => {
    ...
    for (const el of Array.from(document.querySelectorAll(".ds-message"))) {
      const assistantContent = el.querySelector(".ds-assistant-message-main-content, .ds-markdown");
      const role = assistantContent ? "assistant" : "user";
      const contentEl = assistantContent ?? el;
