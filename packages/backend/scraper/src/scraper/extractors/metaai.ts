...
export async function extractMetaAI(page: Page): Promise<ExtractResult> {
  await page.waitForSelector('[data-message-item="true"]', { timeout: 20000 }).catch(() => null);
  ...
  const result = await page.evaluate(() => {
    ...
    function htmlToMarkdown(el: Element): string {
      ...
      for (const junk of Array.from(clone.querySelectorAll('[data-testid="citation-pill"]'))) junk.remove();
    ...
    for (const el of Array.from(document.querySelectorAll('[data-message-item="true"]'))) {
      const role = el.querySelector('[data-message-type="user"]') ? "user" : "assistant";
