...
export async function extractQwen(page: Page): Promise<ExtractResult> {
  await page.waitForSelector(".qwen-chat-message", { timeout: 20000 }).catch(() => null);
  ...
  const result = await page.evaluate(() => {
    ...
    function htmlToMarkdown(el: Element): string {
      ...
      for (const junk of Array.from(clone.querySelectorAll('[class*="action-control"], [class*="message-footer"], button'))) junk.remove();
      ...
      return (clone.textContent ?? "").replace(/^\s*Thinking completed\s*/i, "").replace(/\n{3,}/g, "\n\n").trim();
    }
    ...
    for (const el of Array.from(document.querySelectorAll(".qwen-chat-message"))) {
      const isUser = el.classList.contains("qwen-chat-message-user");
      const role = isUser ? "user" : "assistant";
      const contentEl = isUser ? el.querySelector(".user-message-content") ?? el : el;
