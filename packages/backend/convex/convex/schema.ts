...
export default defineSchema({
	users: defineTable({
		...
		plan: v.optional(v.string()), // free, starter_plan, plus_plan, pro_plan, max_plan
		memoryUsedBytes: v.optional(v.number()),
		memoryLimitBytes: v.optional(v.number()),
		fileStorageUsedBytes: v.optional(v.number()),
	...
	chats: defineTable({
		...
		.index("by_owner_and_imported_at", ["ownerId", "source.importedAt"])
	...
	kodeWebFiles: defineTable({
		...
		.index("by_project_and_version_and_path", ["projectId", "version", "path"])
		.index("by_owner", ["ownerId"]),
	...
	kodeWebBuilds: defineTable({
		...
		.index("by_owner_mode_created", ["ownerId", "mode", "createdAt"]),
	...
	usage: defineTable({
		...
		bucketType: v.union(
			...
			v.literal("month_frontier"), // Pro/Max: frontier-model quota
			v.literal("month_kai"), // K-AI 1.0: separate monthly request budget
			v.literal("month_ai_credits"), // shared metered AI work across product surfaces
			v.literal("month_image"), // K-Image generation count
			v.literal("month_video_seconds"), // K-Video generated seconds
			v.literal("day_kai_search"), // K-AI 1.0: daily web-search budget
			v.literal("month_web_search"), // shared monthly search allowance
	...
	// Realtime voice is metered by active conversation time, separately from
	// message quotas. One usage row exists per user per UTC month.
	realtimeVoiceUsage: defineTable({
		ownerId: v.id("users"),
		monthKey: v.string(),
	...
	realtimeVoiceSessions: defineTable({
		ownerId: v.id("users"),
		model: v.string(),
		planTier: v.union(
			v.literal("starter"), // legacy sessions created before Live moved to Pro
			v.literal("pro"),
			v.literal("max"),
		),
		monthKey: v.string(),
		startedAt: v.number(),
		lastMeteredAt: v.number(),
		usedSeconds: v.number(),
		endedAt: v.optional(v.number()),
		endReason: v.optional(v.string()),
	}).index("by_owner_and_started", ["ownerId", "startedAt"]),
	...
	files: defineTable({
		...
		.index("by_owner", ["ownerId"])
		.index("by_owner_file_type_created", ["ownerId", "fileType", "createdAt"])
		.index("by_chat", ["chatId"])
