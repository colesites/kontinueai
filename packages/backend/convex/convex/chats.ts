			...
			if (monthlyLimit !== null) {
				...
				if (importedChatsThisMonthCount >= monthlyLimit) {
					throw new ConvexError({
						code: "PLAN_IMPORT_LIMIT",
						message: `${PLAN_DEFINITIONS[planTier].name} includes ${monthlyLimit} conversation imports per month. Upgrade for a higher limit or wait for the monthly reset.`,
					});
				}
			}
		}
